...
SYSTEM_PROMPT = """You are a receipt parser. Extract data from food delivery receipt images and return ONLY valid JSON.
...
- subtotal = sum of all item prices before any discount
- discount = positive number (the amount deducted)
- coupon = positive number (the amount deducted), 0 if none
...
def process():
    ...
    try:
        ...
        data = parse_receipt(image_bytes, mime_type)
